...
const prepStories = (data) => {
  ...
  for (const item of temp) {
    ...
    let author_id = Math.ceil(Math.random() * 15);
    let ob = {
      content: item.content,
      state: state,
      title: item.title,
      likes: likes,
      genre: g[Math.ceil(Math.random() * 7)],
      author_id: author_id
    }
    stories.push(ob);
  }
...
const insertStories = (data) => {
  const statement = `INSERT INTO stories(content, state, title, likes, genre, photo_url, author_id) VALUES `;
  let values = '';
  let row = {};

  for (let i = 0; i < data.length - 1; i++) {
    row = data[i];
    let value = `('${row.content}', '${row.state}', '${row.title}', '${row.likes}', '${row.genre}', '${photoUrls[i]}', ${row.author_id})`;
    value += ', ';
    values += value;
  }

  row = data[data.length - 1];
  values += `('${row.content}', '${row.state}', '${row.title}', '${row.likes}', '${row.genre}', '${photoUrls[data.length - 1]}', ${row.author_id})`;
  values += ';';
  return { statement, values };
};
