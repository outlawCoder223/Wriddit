...
module.exports = (db) => {
  ...
  const incompleteStory = (user, id) => {
    ...
    const promise2 = getIncomplete(id);
...
const merge = (storyId, contributionId) => {
    return getComplete(storyId)
      .then((data) => {
        return data.rows[0].content;
      })
      .then((mergeContent) => {
        return db.query(getContributionById, [contributionId])
          .then(data => {
      ...
      .then(mergeContent => {
        return db.query(mergeContribution1, [contributionId])
          .then(() => {
            return db.query(`
        UPDATE stories SET content = $1
        WHERE stories.id = $2;`, [mergeContent, storyId]);
          });
      })
      .then(() => {
        //update all contribution statuses related to that story
        return db.query(mergeContribution2, [storyId]);
      })
      .catch(err => {
        console.log(err);
        res
